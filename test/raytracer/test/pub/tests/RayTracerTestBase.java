...
public abstract class RayTracerTestBase {
	...
	public void checkColorCheckerboardSimple(final Shader s1, final Shader s2, final Color c1,
		...
		Hit hit = plane.hit(ray, checkerboard, 0, Float.MAX_VALUE);
		hit.hits();
		Trace trace = Trace.primary(scene, ray);
		Color c = shader.shade(hit, trace);
		assertTrue(
				"Color should be " + c1.toString() + " but was " + c.toString(),
				c.equals(c1));

		ray = new Ray(new Point(1f, 1f, -10f), Vec3.Z);
		hit = plane.hit(ray, checkerboard, 0, Float.MAX_VALUE);
		hit.hits();
		trace = Trace.primary(scene, ray);
		c = shader.shade(hit, trace);
		assertTrue(
				"Color should be " + c1.toString() + " but was " + c.toString(),
				c.equals(c1));
	...
	public void checkPhong(final Color c1, final Color c2, final Color c3,
		...
		Hit hit = plane.hit(ray, phongPlane, 0, Float.MAX_VALUE);
		hit.hits();
		Trace trace = Trace.primary(scene, ray);
		Color c = shader.shade(hit, trace);
		assertTrue(
		...
		hit = plane.hit(ray, phongPlane, 0, Float.MAX_VALUE);
		hit.hits();
		trace = Trace.primary(scene, ray);
		c = shader.shade(hit, trace);
		assertTrue(
